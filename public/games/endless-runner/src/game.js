...
class Game extends Phaser.Scene {
  ...
  create() {
    ...
    const birdDelay = gameState.lowEnd ? [4000, 7000] : [2500, 5000];
    this.birdCreationTime = this.time.addEvent({
      callback: createBird,
      delay: Phaser.Math.Between(birdDelay[0], birdDelay[1]),
      callbackScope: this,
      loop: true,
    });
    ...
    this.coinCreationTime = this.time.addEvent({
      callback: createCoin,
      delay: gameState.lowEnd ? 1500 : 1000,
      callbackScope: this,
      loop: true,
    });
    ...
    this.spikeCreationTime = this.time.addEvent({
      ...
      delay: gameState.lowEnd ? 7000 : 5000,
      callbackScope: this,
      loop: true,
    });
  ...
  update(time, delta) {
    ...
    if (this.health <= 0) {
      ...
      if (typeof window !== 'undefined' && window.PlayNest?.save) {
        window.PlayNest.save.setHighScore(gameState.score);
      }
    ...
    const pn = typeof window !== 'undefined' ? window.__pnRunnerInput : null;
    const wantJump = (this.cursors && Phaser.Input.Keyboard.JustDown(this.cursors.up))
      || (pn && pn.jump);
    ...
    const wantSlide = (this.cursors && this.cursors.down.isDown) || (pn && pn.slideDown);
    if (wantSlide) {
      if (!this.player.body.touching.down) {
